...
export default function OperationsClient({ scopes, locale }: { scopes: OperationsScope[]; locale: "en" | "he" | "fr-CA" }) {
  ...
  return <section className="operations-content">
    <header className="operations-hero"><div><p>{t.eyebrow}</p><h1>{t.title}</h1><span>{t.intro}</span><a className="operations-new-spa" href="/admin/operations/spas/new">{locale === "he" ? "תצוגת סביבת עבודה לספא חדש" : locale === "fr-CA" ? "Voir l’espace d’un nouveau spa" : "Preview a new spa workspace"}</a></div><div className="operations-scope"><label>{t.live}<select value={scope} onChange={event => setScope(event.target.value as OperationsScope)}>{scopes.map(item => <option key={item} value={item}>{t.scopes[item]}</option>)}</select></label><button onClick={() => void load()}>{t.refresh}</button></div></header>
