...
const copy = {
  en: {
    ...
    languageSection: "Meeting language", languageHelp: "Choose the language for the meeting invitation and the message your guest receives.", hebrew: "עברית", english: "English",
  ...
  he: {
    ...
    languageSection: "שפת הפגישה", languageHelp: "יש לבחור את שפת ההזמנה וההודעה שיקבל האורח.", hebrew: "עברית", english: "English",
...
const defaultMeetingTitle = (locale: Locale) => locale === "he" ? "פגישה עם SpaPlus Global" : "Meeting with SpaPlus Global";
...
export default function MeetingClient({ displayName, locale }: { displayName: string; locale: string }) {
  ...
  function chooseLocale(nextLocale: Locale) {
    if (submitting || bookingId || nextLocale === activeLocale) return;
    setTitle((currentTitle) => currentTitle === defaultMeetingTitle(activeLocale) ? defaultMeetingTitle(nextLocale) : currentTitle);
  ...
  return (
    <main className="meeting-shell" lang={activeLocale} dir={rtl ? "rtl" : "ltr"}>
      ...
      <header className="meeting-header">
        ...
        <div className="meeting-header-actions"><a href="/admin">{t.back}</a></div>
      </header>
      <section className="meeting-layout" aria-labelledby="meeting-title">
        ...
        <div className="meeting-main">
          ...
          <form id="meeting-form" className="meeting-card" onSubmit={submit} aria-busy={submitting}>
            <fieldset className="meeting-language-fieldset"><legend>{t.languageSection}</legend><p>{t.languageHelp}</p><div className="meeting-language-picker" role="radiogroup" aria-label={t.languageSection}><button type="button" role="radio" aria-checked={activeLocale === "he"} className={activeLocale === "he" ? "is-selected" : ""} disabled={submitting || Boolean(bookingId)} onClick={() => chooseLocale("he")}>{t.hebrew}</button><button type="button" role="radio" aria-checked={activeLocale === "en"} className={activeLocale === "en" ? "is-selected" : ""} disabled={submitting || Boolean(bookingId)} onClick={() => chooseLocale("en")}>{t.english}</button></div></fieldset>
