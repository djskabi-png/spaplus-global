...
function brandedGuestEmail(data: {
  ...
  const subject = he ? `הזמנה לפגישה: ${data.title}` : `Meeting invitation: ${data.title}`;
  ...
  const html = `<!doctype html><html lang="${data.locale}" dir="${direction}"><body style="margin:0;background:#f5f7fb;color:#142d4f;font-family:${he ? "Heebo" : "Arial"},Arial,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="padding:32px 16px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#fff;border:1px solid #e2e8f0;border-radius:24px;overflow:hidden"><tr><td style="padding:24px 30px;background:#142d4f"><img src="https://app.spaplus.co/spaplus-wordmark.png" width="116" alt="SpaPlus Global" style="display:block;background:#fff;border-radius:8px;padding:5px 10px"></td></tr><tr><td style="padding:34px 30px"><p style="margin:0 0 10px;color:#e91562;font-size:13px;font-weight:700">SpaPlus Global</p><h1 style="margin:0 0 18px;font-size:28px;line-height:1.25">${escapeHtml(data.title)}</h1><p style="margin:0 0 8px;font-size:17px;line-height:1.7">${escapeHtml(greeting)}</p><p style="margin:0 0 24px;color:#526b84;line-height:1.7">${escapeHtml(intro)}</p><div style="margin-bottom:24px;padding:18px;border-radius:16px;background:#fff4f8"><strong style="display:block;margin-bottom:5px">${escapeHtml(when)}</strong><span style="color:#60778f;font-size:13px">${escapeHtml(data.timeZone)}</span></div><a href="${escapeHtml(data.meetUrl)}" style="display:block;padding:15px 22px;border-radius:12px;background:#e91562;color:#fff;text-align:center;text-decoration:none;font-weight:700">${join}</a><p style="margin:16px 0 0;text-align:center"><a href="${escapeHtml(data.calendarUrl)}" style="color:#294b6b;font-size:13px;font-weight:700">${calendar}</a></p>${note}</td></tr></table></td></tr></table></body></html>`;
